...
def HDBresaleindex(filepath="./raw_data/HDBResalePriceIndex1Q2009100Quarterly.csv"):
    ...
    made some adjustments to the data, and stores the processed data as csv in data folder.
...
def HDBresalemedian(
    ...
    Args:
        filepath (str, optional): location where raw file is stored.
        Defaults to "./raw_data/MedianResalePricesforRegisteredApplicationsbyTownandFlatType.csv".
    """
    df = pd.read_csv(filepath)
...
def HDBresaledetails(
    ...
    Args:
        filepath (str, optional): location where raw file is stored.
        Defaults to "./raw_data/ResaleflatpricesbasedonregistrationdatefromJan2017onwards.csv".
    """
    df = pd.read_csv(filepath)
...
def CEAagenttxn(
    ...
    made some adjustments to the data, combines them and stores the processed data as csv in data folder.

    Args:
        filepath (list, optional): list of locations where raw files are stored.
        Defaults to [ "./raw_data/CEASalespersonInformation.csv", "./raw_data/CEASalespersonsPropertyTransactionRecordsresidential.csv", ].
    """
